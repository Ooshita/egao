...
int H=360,S=100,B=100; //HSB 明るさ調整の為の変数
...
void draw() {
      ...
      //color1の部分
        for(int i=0;i<w*h;i++){//画面全体のピクセル数だけ繰り返し処理
          ...
          float difRed=abs(red(targetColor)-red(video.pixels[i])); //video.pixels[i]はウェブカメラのリアルタイムの映像。
          ...
          //色の差が大きいと違う色になる。色の差が小さいと近似色になるのでそれを利用。
 ...
 //最初に色を指定した後、中点に色を置いてしまうので、そこを今後対応。（対応済み）
        for(int i=0;i<var -1;i++) { //描画処理
            if(color1==true){
              fill(targetColor);//塗り色
              ellipse(w-pointX[i], pointY[i], 25, 25);
            }
        }
...
void keyPressed(){
  ...
  if(key=='i'){
    if(iro>2){//iroが１になったら落ちるため。一以外の時に実行
    iro+=-1;
    }
  }else if (key=='l'){
  ...
  if(key==CODED){
    ...
    if(key==CODED){
    if(keyCode==UP){//左キーを押した場合
    if(H!=255){ //255以下になるとクラッシュしてしまうため。
      H+=-3;
  ...
  if(key=='s'){
    ...
    String s="egao_" + num + ".jpg";//保存ファイル名
    save(s);//画像保存
...
void captureEvent(Capture video) {
  video.read();
  filter(POSTERIZE, iro); //色を強調させている。
  //H：色相　S：彩度　B：明度
  colorMode(HSB, H, S, B);
}
